...
export class Toaster{
  ...
  pop(options:ToasterOptions){
    ...
    let type='info';
    if(options.type){
      let types=['success','error','info','wait','warning','showWait'];
  ...
  private createOptions(type:string,args:any):ToasterOptions{
    ...
    if(['info','success','wait','warning','error','showWait'].indexOf(type)>=0){
      opt.type=type;
    }
    if(args){
      if(args.length>1){
  ...
  info(...args){
    return this.pop(this.createOptions('info',args));
  }
  success(...args){
    return this.pop(this.createOptions('success',args));
  }
  wait(...args){
    return this.pop(this.createOptions('wait',args));
  }
  warning(...args){
    return this.pop(this.createOptions('warning',args));
  }
  error(...args){
    return this.pop(this.createOptions('error',args));
  }
  showWait(...args){
    return this.pop(this.createOptions('showWait',args));
  }
  hideWait(toast:HTMLElement){
